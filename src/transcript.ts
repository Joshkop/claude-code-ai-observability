  ...
  for (const line of raw.split("\n")) {
    ...
    if (parsed.type === "assistant") {
      ...
      if (Array.isArray(parsed.message?.content) && parsed.message!.content!.length > 0) {
        const last = (parsed.message!.content as unknown[])[parsed.message!.content!.length - 1];
        if (last && typeof last === "object") {
          const lt = (last as Record<string, unknown>).type;
          if (lt === "text") current.endsWithToolUse = false;
          else if (lt === "tool_use") current.endsWithToolUse = true;
        }
      }
    }
  }
