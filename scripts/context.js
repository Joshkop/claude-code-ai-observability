...
function run(args, timeoutMs = 2000) {
    return new Promise((resolve, reject) => {
        const [cmd, ...rest] = args;
        const child = exec([cmd, ...rest].map((a) => JSON.stringify(a)).join(" "), { signal: AbortSignal.timeout(timeoutMs), timeout: timeoutMs }, (err, stdout) => {
            if (err)
                reject(err);
            else
                resolve(stdout.trim());
...
export async function detectContext(sessionId, cwd) {
    const effectiveCwd = cwd ?? process.cwd();
    const gitArgs = (subcmd) => ["git", "-C", effectiveCwd, ...subcmd];
    // All detections run in parallel; each is isolated in its own try/catch.
    const [sessionNameResult, versionResult, remoteUrlResult, repoRootResult, branchResult, revisionResult,] = await Promise.allSettled([
        // claude_code.session_name
        (async () => {
            if (process.env.CLAUDE_SESSION_NAME)
                return process.env.CLAUDE_SESSION_NAME;
