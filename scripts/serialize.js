const SENSITIVE_KEY_PATTERN = /(api[-_]?key|token|secret|password|passwd|authorization|cookie|session|bearer|x[-_]?api[-_]?key|access[-_]?key|private[-_]?key|client[-_]?secret|refresh[-_]?token|aws[-_]?secret|gh[-_]?token|github[-_]?token|slack[-_]?token|signing[-_]?key|salt|hmac|jwt|otp|pin|credential)/i;
...
export function serialize(value, maxLength) {
